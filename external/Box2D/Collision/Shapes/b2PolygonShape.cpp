...
void b2PolygonShape::SetAsBox(qreal hx, float32 hy)
{
	m_count = 4;
...
void b2PolygonShape::SetAsBox(qreal hx, float32 hy, const b2Vec2& center, float32 angle)
{
	m_count = 4;
	m_vertices[0].Set(-hx, -hy);
	m_vertices[1].Set( hx, -hy);
...
void b2PolygonShape::ComputeMass(b2MassData* massData, qreal density) const
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		qreal intx2 = ex1*ex1 + ex2*ex1 + ex2*ex2;
		qreal inty2 = ey1*ey1 + ey2*ey1 + ey2*ey2;
